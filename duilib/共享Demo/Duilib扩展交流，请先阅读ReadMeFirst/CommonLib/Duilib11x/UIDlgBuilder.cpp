...
/// \brief 界面构造器创建界面的入口
///
/// Image Font Default作为公共资源，会保存在绘制管理器里。
/// Window是主窗口，其他元素在_Parse里解析
...
/// \brief 解析XML文件，创建界面
///
/// Image Font Default作为公共资源，会保存在绘制管理器里（根节点下的已经解析，其他节点下的跳过）。
/// Include分离出一个子界面，可以是Window也可以是Layout
/// 各种控件和布局可以创建，无法创建的通过插件或回调函数创建。
