...
///////////////////////////////////////////////////////////////////////////////////////
#define Open7zip Open7zipU
#define Close7zip Close7zipU

#ifdef _UNICODE
extern HANDLE Open7zipU(const wchar_t *z, HANDLE h7z = NULL);
#define Find7ZipItem Find7zipItemW
#else
extern HANDLE Open7zipU(const char *z, HANDLE h7z = NULL);
#define Find7ZipItem Find7zipItemA
#endif

extern HANDLE Open7zipU(void *z, unsigned int l, HANDLE h7z = NULL);
extern unsigned long Close7zipU(HANDLE h7z);

extern unsigned long Get7zipItem(HANDLE h7z, int index, unsigned __int64* size);
extern unsigned long Get7zipCount(HANDLE h7z,size_t* size);

extern unsigned long Find7zipItemW(HANDLE h7z, const WCHAR *name, size_t *index, unsigned __int64 *size);
extern unsigned long Find7zipItemA(HANDLE h7z, const CHAR *name, size_t *index, unsigned __int64 *size);

extern unsigned long Un7zipItem(HANDLE h7z, int index, void *dst);
//////////////////////////////////////////////////////////////////////////
...
bool CMarkup::LoadFromFile(LPCTSTR pstrFilename, int encoding)
{
    ...
    if( CPaintManagerUI::GetResourceZip().IsEmpty() ) {
		...
		else
		{
			sFile += CPaintManagerUI::GetResourceZip();
			HANDLE hz = NULL;
			if( CPaintManagerUI::IsCachedResourceZip() ) hz = CPaintManagerUI::GetResourceZipHandle();
			else {
				hz = Open7zip(sFile.GetData());
			}
			if( hz == NULL ) return _Failed(_T("Error opening zip file"));
			unsigned __int64 ze = 0;
			size_t i; 
			if( Find7ZipItem(hz,pstrFilename,&i,&ze) != 0 ) return _Failed(_T("Could not find ziped file"));
			DWORD dwSize = ze;
			if( dwSize == 0 ) return _Failed(_T("File is empty"));
			if ( dwSize > 4096*1024 ) return _Failed(_T("File too large"));
			BYTE* pByte = new BYTE[ dwSize ];
			int res = Un7zipItem(hz,i,pByte);
			if( res != 0x00000000 && res != 0x00000600) {
				delete[] pByte;
				if( !CPaintManagerUI::IsCachedResourceZip() ) Close7zip(hz);
				return _Failed(_T("Could not unzip file"));
			}
			if( !CPaintManagerUI::IsCachedResourceZip() ) Close7zip(hz);
			bool ret = LoadFromMem(pByte, dwSize, encoding);
