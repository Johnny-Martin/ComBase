...
bool CPaintManagerUI::m_bUse7zip = false;
...
bool CPaintManagerUI::IsUse7zip()
{
	return m_bUse7zip;
}
...
void CPaintManagerUI::SetUse7zip(bool bEnable)
{
	if(m_bUse7zip == bEnable) return;
...
void CPaintManagerUI::SetResourceZip(LPVOID pVoid, unsigned int len)
{
    ...
    if( m_bCachedResourceZip ) 
		if(!m_bUse7zip)
			...
			m_hResourceZip = Open7zip(pVoid,len);
}
...
void CPaintManagerUI::SetResourceZip(LPCTSTR pStrPath, bool bCachedResourceZip)
{
    ...
    if( m_bCachedResourceZip ) {
		...
		if(!m_bUse7zip)
			...
			m_hResourceZip = Open7zip(sFile.GetData());
    }
}
